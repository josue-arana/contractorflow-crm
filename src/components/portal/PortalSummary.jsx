...
export function PortalSummary({ lead, portal, full = false, t = (key) => key, portalSettings = {}, paymentLayout = 'default', renderTimelineActions = null }) {
  ...
  const estimateNumber = getEstimateDisplayNumber(portal?.estimate, lead)
                ...
                <div className="min-w-0 flex-1 rounded-2xl bg-slate-50 p-4">
                  <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto">
                      ...
                      {renderTimelineActions?.(item, index)}
                    </div>
                  </div>
      ...
      <div className="space-y-5">
        ...
        <InfoCard title={t('paymentProgress')}>
          {paymentLayout === 'projectWorkspace' ? (
            <>
              <DetailRow label={t('depositPaid')} value={currency.format(portal.depositPaid ?? 0)} />
              <DetailRow label={t('paymentsRecorded')} value={currency.format(portal.otherPaymentsTotal ?? Math.max((portal.totalPaid ?? portal.amountPaid ?? 0) - (portal.depositPaid ?? 0), 0))} />
